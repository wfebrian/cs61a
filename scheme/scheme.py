...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        def to_python_list(scheme_list):
            python_list = []
            while scheme_list != nil:
                python_list.append(scheme_list.car())
                scheme_list = scheme_list.cdr()
            return python_list
        try:
            args_list = to_python_list(args)
            if self.use_env:
                args_list.append(env)
            applied = self.fn(*args_list)
            return (applied, None)
        except TypeError:
            raise SchemeError
...
def do_lambda_form(vals, env, function_type=LambdaProcedure):
    ...
    return (function_type(formals, vals.second.first, env), None)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        name = vals.first.first
        if scheme_symbolp(name):
            args = vals.first.second
            construct_lambda = do_lambda_form(Pair(args, vals.second),env)[0]
            env.define(name, construct_lambda)
            return (name, None)
        raise SchemeError("bad argument to define")
    else:
        raise SchemeError("bad argument to define")
...
def do_or_form(vals, env):
    ...
    if len(vals) == 0:
        return scheme_false, None
    for expr in vals:
        current_val = scheme_eval(expr,env)
        if current_val:
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            ...
            if len(clause.second) == 1:
                ...
                return test, None
            return do_begin_form(clause.second, env)[0], None
    return okay, None
...
def check_formals(formals):
    ...
    for i in range(len(formals)):
        current = formals[i]
        if scheme_symbolp(current):
            if current in tracker:
                raise SchemeError("formals cannot be repeated")
            tracker.append(current)
        elif not scheme_symbolp(current):
            raise SchemeError("formals is not a well-formed symbol")

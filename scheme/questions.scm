...
(define (tree-sums tree)
  ; *** YOUR CODE HERE ***
  (define (tree-total x v-list)
    (define (combine other) (+ x other))
    (map combine v-list)
  )

  (define (flatten-list deep-list)
    (if (null? deep-list) nil (append (car deep-list) (flatten-list (cdr deep-list))))
  )

  (if (null? (children tree)) (cons (entry tree) nil) 
    (tree-total (entry tree) (flatten-list (map tree-sums (children tree)))))
)

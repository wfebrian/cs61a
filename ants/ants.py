...
# Name: Nick Dill, Wesley Febrian
# Email: nickdill@berkeley.edu, wfebrian@berkeley.edu
...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            if type(insect) == BodyguardAnt and insect.ant is not None:
                self.ant = insect.ant
            elif type(insect) == QueenAnt and insect.is_impostor == False:
                return
            else:
                self.ant = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        x = 0
        while x < self.max_range and current_place.entrance and current_place.entrance.name != 'Hive' and len(current_place.bees) == 0:
            current_place = current_place.entrance
            x += 1
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        QueenAnt.queen_ant_number += 1
        if QueenAnt.queen_ant_number == 1:
    ...
    def action(self, colony):
        ...
        elif not self.is_impostor:
            ScubaThrower.action(self, colony)
            colony.queen = QueenPlace(colony.queen, self.place)

            def damage_doubler():
                if current_place.ant != None and current_place.ant != self and (current_place.ant not in QueenAnt.doubled_ants):
                    QueenAnt.doubled_ants.append(current_place.ant)
                    current_place.ant.damage *= 2
                    if current_place.ant.container and current_place.ant.ant != None and current_place.ant.ant != self and (current_place.ant.ant not in QueenAnt.doubled_ants):
                        QueenAnt.doubled_ants.append(current_place.ant.ant)
